...
# Assuming Point class exists based on your original code
class Point:
...
class StaticParticleFilterTDOA:
    def __init__(self, receivers, base_variance, num_particles=1000, c_r=0.01, c_q=0.01):
        ...
        # Initialize N random particles (Step 1)
        self.particles = [
            Point("Particle", np.random.uniform(0, 1), np.random.uniform(0, 1), np.random.uniform(0, 1))
            for _ in range(num_particles)
        ]
        
    def _build_base_covariance_matrix(self):
        """Builds the (N-1)x(N-1) covariance matrix anchored to receiver 0."""
    ...
    def run_filter(self, delta_d, iterations=100):
        """
        Executes Algorithm 2 until convergence (or max iterations).
        ...
        for k in range(1, iterations + 1):
            ...
            # R_bar = R + (C_R / k^2) * Identity Matrix
            ...
            # Step 3(a): Compute particle weights using Mahalanobis distance
            for i, p in enumerate(self.particles):
                ...
                # Added -0.5 to correct the algorithm image to standard Gaussian
            ...
            # Step 3(b): Compute the estimate P_hat
            ...
            # Step 3(d): Spread out the particles P_i = P_i + w
            std_dev = np.sqrt(Q_bar)
            new_particles = []
            
            for idx in indices:
                old_p = self.particles[idx]
                # Applying random walk covariance Q_bar
        ...
        return Point("Estimate", ex, ey, ez)
